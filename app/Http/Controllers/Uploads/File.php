...
class File extends UploadCore
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $fileMaxSize = 1024 * 1024 * 5; // 5MB Custom file size
        $extensionMImes = implode(',', $this->fileExtensionType('document_image'));
        ...
        foreach ($files as $file) {
            if ($file->getSize() > $fileMaxSize) {
                return $this->response(ResponseCode::HTTP_BAD_REQUEST, 'File size is too large');
            }

            if ($file->extension()) {
                $extension = $file->extension();
                $extensionMImesArr = explode(',', $extensionMImes);
                if (!in_array($extension, $extensionMImesArr)) {
                    return $this->response(ResponseCode::HTTP_BAD_REQUEST, 'File type not allowed');
                }
            }
        }
        ...
        foreach ($files as $file) {
            ...
            if ($type_file == 'image') {
                ...
                if ($image->width() > 720) {
                    $image->scaleDown(width: 720); // resize the image to a width of 200 and constrain aspect ratio (auto height)

                }

                $pathFileWithoutExt = Str::of($filePath)->replaceEnd('.' . $extension, '');
                $pathThumb = $pathFileWithoutExt . '_thumb.' . $extension;
                $image->save($pathThumb);

                $itemData['url'] = config('api.endpoint_file') . $access_code . '/' . $fid . '/thumb';
                $itemData['size'] = filesize($pathThumb);
                $dataFiles[] = $itemData;
            }
